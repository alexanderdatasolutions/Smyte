...
- [ ] **Wire LootSystem save methods into SaveManager** — LootSystem has `get_save_data()`/`load_save_data()` methods but SaveManager never calls them. Orphaned save data. `{"file": "scripts/systems/resources/LootSystem.gd", "passes": false}`
...
- [ ] **Remove dead summon functions from SummonManager.gd** — `summon_with_element_soul()`, `summon_with_powder()`, `summon_basic_with_powder()`, `get_powder_cost()`, `get_powder_weight_multiplier()`, `can_afford_powder_summon()`, `grant_element_favor()`, `get_element_favor_status()`, `can_use_weekly_premium_summon()`, `summon_multi_with_soul()` (all 0 external callers). `{"file": "scripts/systems/collection/SummonManager.gd", "passes": false}`
- [ ] **Remove dead functions from ConfigurationManager.gd** — `get_pantheons_config()`, `is_pantheon_enabled()`, `is_configuration_loaded()`, `reload_configurations()` (all 0 external callers). `{"file": "scripts/systems/core/ConfigurationManager.gd", "passes": false}`
- [ ] **Remove dead convenience functions from EventBus.gd** — `emit_resource_change()`, `emit_battle_ended()` (0 external callers). `{"file": "scripts/systems/core/EventBus.gd", "passes": false}`
- [ ] **Remove dead function from SaveManager.gd** — `get_save_info()` (0 external callers). `{"file": "scripts/systems/core/SaveManager.gd", "passes": false}`
