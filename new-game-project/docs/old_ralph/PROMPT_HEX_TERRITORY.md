# Implement ONE Task from Hex Territory System

Read the CLAUDE.md section below for architecture, code standards, and design philosophy.
Read the hex_territory_activity.md section below to see what was recently accomplished.

## Find Your Task

Find the FIRST task in hex_territory_plan.md (included below) where `"complete": false`.
...
1. Read existing patterns: TraitManager.gd, CollectionManager.gd, TerritoryManager.gd
...
### For JSON Data (data/):
1. Read existing patterns: traits.json, gods.json, territories.json
2. Create comprehensive data (not placeholders)
3. Validate JSON syntax
4. Follow schema from CLAUDE.md

### For UI (scripts/ui/):
1. Read existing patterns in scripts/ui/screens/ and scripts/ui/territory/
2. Follow dark fantasy theme
3. Match existing styling patterns
4. Use coordinator pattern for complex screens

## Hex Grid Specifics

### Axial Coordinates
Use (q, r) coordinate system for hex positions:
- q = column, r = row
...
### Ring Structure
- Ring 0: Base at (0,0)
- Ring 1: 6 nodes around base (distance 1)
- Ring 2: 12 nodes (distance 2)
- Ring N: 6*N nodes

### Node Tier Rules
- Tier 1: No spec required
- Tier 2: Any Tier 1 spec required
- Tier 3: Any Tier 2 spec required
- Tier 4: Tier 2 spec matching node type required
- Tier 5: Any Tier 3 spec required

## After Implementation

1. Take a screenshot of the game to verify nothing is broken (if UI changed)
2. Update the task's `complete` in smyte-ralph/hex_territory_plan.md from `false` to `true`
3. Append a dated entry to smyte-ralph/hex_territory_activity.md:
   ```
...
4. Make one git commit:
   ```bash
   cd ~/Documents/Coding/Smyte && git add -A && git commit -m "feat(hex): [brief description]"
   ```
...
- CLAUDE.md - Master design document (has hex grid design)
- scripts/data/Territory.gd - Existing territory data class
- scripts/systems/territory/TerritoryManager.gd - Existing territory logic
- scripts/data/GodSpecialization.gd - Specialization data pattern
- scripts/systems/specialization/SpecializationManager.gd - Spec checking pattern
- data/specializations.json - Specialization data example
