...
func _input(event: InputEvent) -> void:
	if event is InputEventKey and event.pressed:
		# F10: Boost first legendary god to level 40
		if event.keycode == KEY_F10:
			_debug_boost_legendary_to_40()
		# F9: Give 100k XP to highest level god
		elif event.keycode == KEY_F9:
			_debug_give_xp_to_first_god(100000)
		# F8: Force unlock skin_selection feature
...
func _debug_grant_zeus_skin() -> void:
	var skin_manager = _get_system("SkinManager")
	var collection_manager = _get_system("CollectionManager")
	if not skin_manager or not collection_manager:
		print("DEBUG: Systems not found")
		return
	...
	# Grant zeus_beach_bod skin
	var skin_id: String = "zeus_beach_bod"
	if skin_manager.has_method("grant_skin"):
		skin_manager.grant_skin(skin_id)
		print("DEBUG: Granted skin '%s' to player" % skin_id)

		# Auto-equip it
		if skin_manager.has_method("equip_skin"):
			skin_manager.equip_skin(zeus.id, skin_id)
			print("DEBUG: Equipped skin '%s' on %s" % [skin_id, zeus.name])
	else:
		print("DEBUG: SkinManager doesn't have grant_skin method")


func _debug_boost_legendary_to_40() -> void:
	var collection_manager = _get_system("CollectionManager")
	var god_progression = _get_system("GodProgressionManager")
	...
	var gods: Array = collection_manager.get_all_gods()
	for god: God in gods:
		if god.tier == God.TierType.LEGENDARY:  # Legendary = 3 in enum
			god_progression.debug_set_god_level(god, 40)
			print("DEBUG: Boosted %s (legendary) to level 40" % god.name)
			return

	print("DEBUG: No legendary god found in collection")


func _debug_give_xp_to_first_god(amount: int) -> void:
	var collection_manager = _get_system("CollectionManager")
	var god_progression = _get_system("GodProgressionManager")
	if not collection_manager or not god_progression:
		print("DEBUG: Systems not found")
	...
	var gods: Array = collection_manager.get_all_gods()
	if gods.size() > 0:
		# Find highest level god
		var highest_god: God = gods[0]
		for god: God in gods:
			if god.level > highest_god.level:
				highest_god = god
		god_progression.add_experience_to_god(highest_god, amount)
		print("DEBUG: Gave %d XP to %s (now level %d)" % [amount, highest_god.name, highest_god.level])


func _debug_unlock_skins() -> void:
	var feature_manager = _get_system("FeatureUnlockManager")
	var achievement_manager = _get_system("AchievementManager")

	if feature_manager and feature_manager.has_method("unlock_feature"):
		feature_manager.unlock_feature("skin_selection")
		print("DEBUG: Unlocked skin_selection feature")

	if achievement_manager and achievement_manager.has_method("complete_achievement"):
		achievement_manager.complete_achievement("legendary_champion")
		print("DEBUG: Completed legendary_champion achievement")
