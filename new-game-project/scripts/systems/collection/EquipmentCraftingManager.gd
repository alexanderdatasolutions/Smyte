...
func _ready():
	...
	print("EquipmentCraftingManager: Initialized")
...
func load_crafting_config():
	...
	if system_registry:
		...
		if config_manager:
			...
			print("EquipmentCraftingManager: Loaded configs through SystemRegistry")
			return
...
func craft_equipment(recipe_id: String, crafting_god_id: String = "", territory_id: String = "") -> Equipment:
	...
	print("EquipmentCraftingManager: Successfully crafted %s" % equipment.get_display_name())
	return equipment
	...
	if system_registry:
		...
		if collection_manager:
			...
			if god and god.level > 1:
				...
				print("EquipmentCraftingManager: Crafting god %s gained %d exp" % [god.name, exp_gained])
...
func unlock_recipe(recipe_id: String):
	...
	print("EquipmentCraftingManager: Recipe unlocked - %s" % recipe_id)

# === TESTING METHODS ===

func _test_crafting_operations():
	"""Test crafting operations"""
	print("EquipmentCraftingManager: Running tests...")
	
	var test_recipes = get_all_recipes()
	if test_recipes.size() > 0:
		var test_recipe = test_recipes[0]
		var craft_check = can_craft_equipment(test_recipe)
		print("EquipmentCraftingManager: Test recipe check result: ", craft_check)
	
	print("EquipmentCraftingManager: Tests completed!")
