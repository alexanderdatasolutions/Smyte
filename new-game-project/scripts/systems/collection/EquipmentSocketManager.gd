...
func _ready():
	...
	load_socket_config()
	print("EquipmentSocketManager: Initialized")
...
func load_socket_config():
	...
	if system_registry:
		...
		if config_manager:
			...
			print("EquipmentSocketManager: Loaded equipment config through SystemRegistry")
			return
...
func unlock_socket(equipment: Equipment, socket_index: int) -> bool:
	...
	if not equipment.can_unlock_socket(socket_index):
		print("EquipmentSocketManager: Cannot unlock socket %d on %s" % [socket_index, equipment.get_display_name()])
	...
	if not _can_afford_socket_cost(unlock_cost):
		print("EquipmentSocketManager: Cannot afford socket unlock cost")
	...
	print("EquipmentSocketManager: Unlocked socket %d on %s (%s type)" % [socket_index, equipment.get_display_name(), socket_type])
	return true
...
func socket_gem(equipment: Equipment, socket_index: int, gem_id: String) -> bool:
	...
	if not socket.get("unlocked", false):
		print("EquipmentSocketManager: Socket %d not unlocked" % socket_index)
		return false
	
	if not socket.gem.is_empty():
		print("EquipmentSocketManager: Socket %d already occupied" % socket_index)
		return false
	
	# Check if player has the gem
	if not _has_gem_in_inventory(gem_id):
		print("EquipmentSocketManager: Gem %s not found in inventory" % gem_id)
		return false
	
	# Check compatibility
	if not _is_gem_compatible_with_socket(gem_id, socket.type):
		print("EquipmentSocketManager: Gem %s not compatible with socket type %s" % [gem_id, socket.type])
		return false
	...
	print("EquipmentSocketManager: Socketed %s into %s socket %d" % [gem_id, equipment.get_display_name(), socket_index])
	return true
...
func unsocket_gem(equipment: Equipment, socket_index: int) -> Dictionary:
	...
	print("EquipmentSocketManager: Removed gem from %s socket %d" % [equipment.get_display_name(), socket_index])
	return gem
...
func add_gem_to_inventory(gem_id: String, quantity: int = 1):
	...
	print("EquipmentSocketManager: Added %d x %s to gem inventory" % [quantity, gem_id])
...
func _test_socket_operations():
	...
	print("EquipmentSocketManager: Running tests...")
	
	# Create test equipment
	var test_equipment = Equipment.create_from_dungeon("test_weapon", "WEAPON", "COMMON", 1)
	if test_equipment:
		var socket_preview = get_socket_upgrade_cost_preview(test_equipment)
		print("EquipmentSocketManager: Test socket preview result: ", socket_preview)
		
		# Test gem inventory
		add_gem_to_inventory("test_gem", 1)
		var gem_count = get_gem_count("test_gem")
		print("EquipmentSocketManager: Test gem count: ", gem_count)
	
	print("EquipmentSocketManager: Tests completed!")
