...
func _ready():
	...
	print("EquipmentEnhancementManager: Initialized")
...
func load_enhancement_config():
	...
	if system_registry:
		...
		if config_manager:
			...
			print("EquipmentEnhancementManager: Loaded equipment config through SystemRegistry")
			return
...
func enhance_equipment(equipment: Equipment, use_blessed_oil: bool = false) -> bool:
	...
	if not equipment.can_be_enhanced():
		print("EquipmentEnhancementManager: Equipment cannot be enhanced further")
		return false
	
	var enhancement_cost = equipment.get_enhancement_cost()
	if not _can_afford_enhancement_cost(enhancement_cost):
		print("EquipmentEnhancementManager: Cannot afford enhancement cost")
		return false
	...
	if use_blessed_oil:
		if not _has_blessed_oil():
			print("EquipmentEnhancementManager: No blessed oil available")
			return false
	...
	if success:
		...
		print("EquipmentEnhancementManager: Enhancement successful! %s now +%d" % [equipment.get_display_name(), equipment.enhancement_level])
	else:
		print("EquipmentEnhancementManager: Enhancement failed!")
...
func _handle_enhancement_failure(equipment: Equipment, used_blessed_oil: bool):
	...
	if consequence.begins_with("level_reset_chance_"):
		...
		if randf() <= chance:
			var old_level = equipment.enhancement_level
			equipment.enhancement_level = 0
			print("EquipmentEnhancementManager: Enhancement level reset! %s went from +%d to +0" % [equipment.get_display_name(), old_level])
		...
		if randf() <= chance:
			...
			print("EquipmentEnhancementManager: Enhancement level decreased! %s went from +%d to +%d" % [equipment.get_display_name(), old_level, equipment.enhancement_level])
		...
		print("EquipmentEnhancementManager: Equipment destroyed! %s has been lost" % equipment.get_display_name())
...
func _test_enhancement_operations():
	...
	print("EquipmentEnhancementManager: Running tests...")
	
	# Create test equipment
	var test_equipment = Equipment.create_from_dungeon("test_weapon", "WEAPON", "COMMON", 1)
	if test_equipment:
		var preview = get_enhancement_preview(test_equipment)
		print("EquipmentEnhancementManager: Test preview result: ", preview)
		
		var stats = get_enhancement_statistics(test_equipment)
		print("EquipmentEnhancementManager: Test statistics result: ", stats.keys())
	
	print("EquipmentEnhancementManager: Tests completed!")
