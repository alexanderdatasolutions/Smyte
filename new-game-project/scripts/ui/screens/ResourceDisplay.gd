...
func _update_mana_display():
	...
	if mana_label:
		...
		var mana_value = resource_manager.get_resource("mana") if resource_manager else 0
...
func _update_crystals_display():
	...
	if crystal_label:
		var system_registry = SystemRegistry.get_instance()
		var resource_manager = system_registry.get_system("ResourceManager") if system_registry else null
		var crystals_value = resource_manager.get_resource("divine_crystals") if resource_manager else 0
...
func _update_tickets_display():
	...
	if tickets_label:
		var system_registry = SystemRegistry.get_instance()
		var resource_manager = system_registry.get_system("ResourceManager") if system_registry else null
		var tickets_count = resource_manager.get_resource("summon_tickets") if resource_manager else 0
...
func _update_energy_display():
	...
	if energy_label:
		var system_registry = SystemRegistry.get_instance()
		var resource_manager = system_registry.get_system("ResourceManager") if system_registry else null
		var energy_value = resource_manager.get_resource("energy") if resource_manager else 0
		var energy_limit = resource_manager.get_resource_limit("energy") if resource_manager else 100
