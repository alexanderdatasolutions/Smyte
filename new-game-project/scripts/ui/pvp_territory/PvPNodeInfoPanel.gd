...
func _update_defense_preview() -> void:
	"""Update defense preview for enemy/neutral nodes"""
	...
	if current_node.is_neutral():
		...
		# Show enemy player's defense team
		var enemy_label := Label.new()
		enemy_label.text = "🛡️ %s's Defense" % current_node.controller_display_name
		...
		if current_node.has_defense_team():
			# Show defense team cards
			var team_row := HBoxContainer.new()
			team_row.add_theme_constant_override("separation", 6)
			_defense_preview_container.add_child(team_row)

			for god_data: Dictionary in current_node.defense_team_serialized:
				var card := _create_god_preview_card(god_data)
				team_row.add_child(card)

			var power_label := Label.new()
			power_label.text = "⚔️ Defense Power: %d" % current_node.defense_power
			power_label.add_theme_font_size_override("font_size", 12)
			power_label.add_theme_color_override("font_color", Color(0.9, 0.6, 0.6))
			_defense_preview_container.add_child(power_label)
		else:
